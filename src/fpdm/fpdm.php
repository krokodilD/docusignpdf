...
namespace Daniilkrok\Docusignpdf\Fpdm;

global $FPDM_FILTERS, $FPDM_REGEXPS;
$FPDM_FILTERS=array(
    'ASCII85Decode',
    'ASCIIHexDecode',
    'FlateDecode',
...
if (!call_user_func_array('class_exists', $__tmp)) {
    ...
    define('FPDM_LOG_PATH', __DIR__."/logs/error.log"); //path to log file

    class FPDM {
    //@@@@@@@@@
	

        var $pdf_source_original;      //string: full pathname to the original input pdf , a form file
		...
		var $err_fields;			//array, showing which fields not filled
    	...
    	function __construct() {
            ...
            switch($num_args) {
                ...
                case FPDM_COMMON: //Common use
                    ...
                    $this->pdf_source_original = $args[0];//Blank pdf form
         ...
         *
         * EXAMPLE
         * Linux: pdftk OR /usr/local/bin/pdftk
         * Windows: C:\Program Files (x86)\PDFtk Server\bin\pdftk.exe
         *
         */
        function processingPDF($pdftkPath) {

            if($this->is_windows()) {
                $cmd_command = '"'.$this->fix_path($pdftkPath).'"'; //For windows
            }else{
                $cmd_command = $this->fix_path($pdftkPath); //For linux and mac
            }

            $inputFilePath = $this->pdf_source;
            $tmpFilePath = sys_get_temp_dir().'/'.uniqid('fpdm_', true) . '.pdf';
            $cmd = $cmd_command.' '.$inputFilePath.' output '.$tmpFilePath;
...
//							$this->set_field_value("tooltip",$name,$value);
                    ...
                    if ($this->err_fields) {
                        $nf_fields = '['.implode(', ', $this->err_fields).']';
                        $msg = '('.date('d/m/Y H:i:s').') Not found fields: ' . $nf_fields . ' in |'.$this->pdf_source_original.'|';
                        error_log($msg."\n", 3, FPDM_LOG_PATH);
                    }
					//===========================================================
